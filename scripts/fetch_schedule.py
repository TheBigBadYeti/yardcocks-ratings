...
def parse(sched):
    ...
    return team_games, probables


def write_caches(outdir, wk_start, wk_end, season_end, wk_games, rest_games, probables):
    ...
    with open(tpath, "w", newline="") as fh:
        ...
        w.writerow(["team", "games_this_week", "remaining_games_season",
                    "week_start", "week_end", "season_end"])
        for t in teams:
            w.writerow([t, wk_games.get(t, 0), rest_games.get(t, 0),
                        wk_start, wk_end, season_end])
...
def main():
    ...
    wk_games, probables = parse(week_sched)
    rest_games, _ = parse(rest_sched)
    tpath, ppath, teams = write_caches(a.outdir, wk_start, wk_end, season_end,
                                       wk_games, rest_games, probables)
